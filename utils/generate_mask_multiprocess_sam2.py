...
from sam2.model_builder import build_sam2_video_predictor
...
def parse_args():
    parser = ArgumentParser(description="Generate SAM2 masks for a VGGSound split.")
    ...
    parser.add_argument(
        "--prompt",
        default="",
        help='Text prompt passed to SAM2. Defaults to empty string ("") for visual prompting style.',
    )
    parser.add_argument(
        "--chunk-index",
        type=int,
...
def process_video(video_file, predictor, prompt):
    ...
    try:
        ...
        # SAM2 requires at least one prompt; use empty text to match SAM-Audio visual prompting style.
